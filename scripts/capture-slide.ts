...
if (!slideArg) {
  console.error('Usage: npx ts-node scripts/capture-slide.ts <slideNumber> [outPath]');
  process.exit(1);
}
...
const outPath = process.argv[3] ? path.resolve(process.argv[3]) : path.join(outDir, defaultName);
